...
eggnog_part <- read_ipc_file("part_humcover3.ipc")
eggnog_fc <- read_tsv("eggnog_fc.tsv")
eggnog_pn <- eggnog_part %>% group_by(hum_protein, Lineage) %>% nest()
eggnog_pc <- eggnog_pn %>% ungroup() %>% group_by(hum_protein) %>% count(name = "nPart")


uniprot_annotations <- read_tsv("../analyses/uniprotkb_AND_model_organism_9606_2024_10_24.tsv.gz")
...
eggnog_upr <- left_join(eggnog_all, uniprot_annotations, by=c("Entry", "Entry Name"))

eggnog_upr_full <- left_join(uniprot_annotations, eggnog_all) %>%
  replace_na(list(nFull = 0, nPart = 0))


write_csv(
  eggnog_upr_full %>% filter(nPart > nFull) %>% select(Entry, `Entry Name`, `Protein names`, nFull, nPart) %>% mutate(`Protein names`=gsub("^([^\\[\\(]*) [\\[\\(].*", "\\1", `Protein names`)) %>% arrange(-nPart),
  "Table1.csv")
...
pdf("supplementary-mitochondrial-enr.pdf")
...
mostly_full_vs_everything <- get_go_enrichment(mostly_full,
                                               all_genes,
                                               desc="Proteins with mostly full-length homologs vs. UniProt",
                                               write_to="mostly_full_vs_everything_GO.csv")
mostly_full_vs_everything_no_mito <- get_go_enrichment(mostly_full_no_mito,
                                               all_genes_no_mito,
                                               desc="Proteins with mostly full-length homologs vs. UniProt",
                                               write_to="mostly_full_vs_everything_GO.csv")
mostly_split_vs_everything <- get_go_enrichment(mostly_split,
                                             all_genes,
                                             desc="Proteins with mostly split homologs vs. UniProt",
                                             write_to="mostly_split_vs_everything_GO.csv")
# mostly_full_vs_ortho <- get_go_enrichment(mostly_full,
#                                           all_ortho_genes,
#                                           desc="Proteins with mostly full-length homologs vs. proteins w/ homologs",
#                                           write_to="mostly_full_vs_everything_GO.csv")
# mostly_split_vs_ortho <- get_go_enrichment(mostly_split,
#                                         all_ortho_genes,
#                                         desc="Proteins with mostly split homologs vs. proteins w/ homologs",
#                                         write_to="mostly_split_vs_everything_GO.csv")
# mostly_split_vs_ortho_MF <- get_go_enrichment(mostly_split,
#                                         all_ortho_genes,
#                                         onto="MF",
#                                         mapping=upr_mf_GO_mapping,
#                                         desc="Proteins with mostly split homologs vs. proteins w/ homologs",
#                                         write_to="mostly_split_vs_everything_GO.csv")
...
write_tsv(mfe_sig,
          "tbl-full-vs-all-enr.tsv")
write_tsv(mfe_nm_sig,
          "tbl-full-vs-all-no-mito-enr.tsv")
write_tsv(msplit_sig,
          "tbl-split-vs-all-enr.tsv")
write_tsv(mfe_sig,
          "SuppTable1.tsv")
write_tsv(msplit_sig,
          "SuppTable2.tsv")
...
library(ggtreeExtra)
library(ggnewscale)
full_best <- open_dataset("full_humcover3.ipc", format="ipc") # note, some non-overlap here??
...
wb$save("Table2_Redo.xlsx")
save_as_docx(pretty_print_table2(table2a) %>% fit_to_width(max_width=7.25, unit="in"), path = "Table2.docx")
save_as_docx(pretty_print_table2(table2b) %>% fit_to_width(max_width=7.25, unit="in"), path = "Table3.docx")
...
pdf(width=10,height=10,"xenobiotic_types.pdf")
print(ggtr_f)
dev.off()
...
genes_mostly_split_display <- eggnog_mostly_split %>%
...
pdf(width=6.5, height=6.5, "Figure2.pdf")
...
pdf(width=10,height=10,"xenobiotic_split_types.pdf")
print(ggtr_split_f2)
dev.off()
