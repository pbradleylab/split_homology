...
main <- function(opt) {
  ...
  if (opt$test) {
    pangenomes = sample(pangenomes, 200)
  }
  tags <- basename(dirname(dirname(dirname(pangenomes))))
  ut <- unique(tags)
  # loop through pangenome locus files
  #for (gi in 1:length(pangenomes)) {
  lapply(1:length(pangenomes), function(gi) {
    # collect garbage every 25
    if ((gi %% 25) == 0) { gc() }
    ...
    # performance reasons
    write_dataset(pg_trim_count,
                  opt$output,
                  format="arrow",
                  existing_data_behavior="overwrite",
                  basename_template=paste0("part-{i}-", gn, ".arrow"))
  })
}
